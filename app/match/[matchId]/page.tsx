    ...
    <div className="mx-auto w-full max-w-3xl px-4 pb-16 pt-8">
      ...
      <section className="panel rise rise-1 mt-3 px-5 py-6 text-center">
        ...
        <div className="mx-auto mt-5 max-w-sm">
          ...
          <div className="scoreboard mt-1 flex justify-between text-sm font-semibold text-muted">
      ...
      {players.map((p, i) => (
        <div key={`${p.spId}-${i}`} className="flex items-center gap-2.5 py-2">
          ...
          <span className="min-w-0 flex-1 truncate text-sm">
            ...
            {(p.status?.assist ?? 0) > 0 && (
              <span className="ml-1 text-sm text-muted">A{p.status.assist}</span>
            )}
          </span>
          <span
