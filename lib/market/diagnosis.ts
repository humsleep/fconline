...
import { BP_REDENOM } from "../nexon/bp";
...
// 화폐개혁(옛 1억 BP = 새 1 BP) 반영. getUserTrades가 이미 새 화폐로 환산해 넘기므로
// 임계값 단위도 같은 배율(BP_REDENOM=1e8)로 축소한다 — 옛 '조/경'을 새 화폐로 표현.
// 결과적으로 옛 1조↔새 1만, 옛 1경↔새 1억 이라 각 규칙의 배수(예: 100*JO)는 그대로 유지되고
// 라벨의 단위 글자만 조→만·경→억으로 바뀐다(정합).
const JO = 1e12 / BP_REDENOM; // 옛 1조 → 새 1만
const GYEONG = 1e16 / BP_REDENOM; // 옛 1경 → 새 1억
...
export const MARKET_RULES: MarketRule[] = [
  ...
  S("t-whale-surplus", "type", "gold", "흑자 큰손", "억 단위를 굴리면서도 순수지가 흑자 — 시장을 지배하는 트레이더입니다.", (s) => s.totalBuy >= 1 * GYEONG && s.net > 0),
  S("t-whale-deficit", "type", "gold", "화끈한 큰손", "억 단위 지출을 아끼지 않는 승부사. 스쿼드가 답을 줄 차례입니다.", (s) => s.totalBuy >= 1 * GYEONG && s.net <= 0),
  ...
  S("n-spend-t1", "note", "info", "소액 지출", "총 영입 지출 1만 미만 — 시장을 조심스럽게 탐색 중입니다.", (s) => s.buyCount > 0 && s.totalBuy < 1 * JO),
  S("n-spend-t2", "note", "info", "가벼운 지출", "총 영입 지출 1~10만 — 필요한 자리만 채우는 실속형.", (s) => s.totalBuy >= 1 * JO && s.totalBuy < 10 * JO),
  S("n-spend-t3", "note", "info", "중간 지출", "총 영입 지출 10~100만 — 로테이션까지 챙기기 시작했군요.", (s) => s.totalBuy >= 10 * JO && s.totalBuy < 100 * JO),
  S("n-spend-t4", "note", "gold", "큰 지출", "총 영입 지출 100~1,000만 — 주전 라인업 대공사 규모입니다.", (s) => s.totalBuy >= 100 * JO && s.totalBuy < 1000 * JO),
  S("n-spend-t5", "note", "gold", "초대형 지출", "총 영입 지출 1,000만~1억 — 리그에서 소문나는 수준의 투자.", (s) => s.totalBuy >= 1000 * JO && s.totalBuy < 1 * GYEONG),
  S("n-spend-t6", "note", "gold", "천문학적 지출", "총 영입 지출 1억 이상 — 이적시장의 큰손입니다.", (s) => s.totalBuy >= 1 * GYEONG),

  // ═══════════ 총수입 (note) ═══════════
  S("n-income-0", "note", "info", "방출 제로", "판 선수가 없습니다 — 정든 선수는 못 보내는 타입인가요?", (s) => s.sellCount === 0 && s.buyCount > 0),
  S("n-income-t1", "note", "info", "소소한 수입", "방출 수입 1만 미만 — 정리보다는 유지에 가깝습니다.", (s) => s.sellCount > 0 && s.totalSell < 1 * JO),
  S("n-income-t2", "note", "info", "쏠쏠한 수입", "방출 수입 1~10만 — 벤치 정리로 용돈벌이 성공.", (s) => s.totalSell >= 1 * JO && s.totalSell < 10 * JO),
  S("n-income-t3", "note", "win", "짭짤한 수입", "방출 수입 10~100만 — 파는 감각이 있습니다.", (s) => s.totalSell >= 10 * JO && s.totalSell < 100 * JO),
  S("n-income-t4", "note", "win", "대형 수입", "방출 수입 100~1,000만 — 셀링 클럽의 풍모.", (s) => s.totalSell >= 100 * JO && s.totalSell < 1000 * JO),
  S("n-income-t5", "note", "win", "초대형 수입", "방출 수입 1,000만 이상 — 팔 때를 아는 구단주.", (s) => s.totalSell >= 1000 * JO),

  // ═══════════ 순수지 (note) ═══════════
  S("n-net-bigplus", "note", "win", "대흑자", "순수지 +1,000만 이상 — 이적시장이 당신의 수익원입니다.", (s) => s.net >= 1000 * JO),
  ...
  S("n-net-bigminus", "note", "lose", "대적자", "순수지 -1,000만 이하 — 스쿼드가 그 값을 해줘야 합니다.", (s) => s.net <= -1000 * JO),

  // ═══════════ 최고가 영입 (note) ═══════════
  S("n-maxbuy-t1", "note", "info", "신중한 최고가", "가장 비싼 영입이 10만 미만 — 고가 카드엔 아직 신중합니다.", (s) => s.maxBuy > 0 && s.maxBuy < 10 * JO),
  S("n-maxbuy-t2", "note", "info", "중형 최고가", "최고가 영입 10~50만 — 핵심 자리엔 지갑을 엽니다.", (s) => s.maxBuy >= 10 * JO && s.maxBuy < 50 * JO),
  S("n-maxbuy-t3", "note", "gold", "대형 영입 한 방", "최고가 영입 50~200만 — 팀의 얼굴이 될 카드군요.", (s) => s.maxBuy >= 50 * JO && s.maxBuy < 200 * JO),
  S("n-maxbuy-t4", "note", "gold", "슈퍼스타 영입", "최고가 영입 200만 이상 — 리그 최고 수준의 베팅입니다.", (s) => s.maxBuy >= 200 * JO),
  S("n-maxbuy-half", "note", "gold", "몰빵 영입", "단 한 건이 전체 지출의 절반 이상 — 확실한 에이스 픽.", (s) => s.buyCount >= 3 && s.maxBuy >= s.totalBuy * 0.5),

  // ═══════════ 최고가 방출 (note) ═══════════
  S("n-maxsell-t1", "note", "info", "잔잔한 방출", "최고가 방출이 10만 미만 — 고가 카드는 아직 품고 있습니다.", (s) => s.maxSell > 0 && s.maxSell < 10 * JO),
  S("n-maxsell-t2", "note", "win", "중형 방출", "최고가 방출 10~50만 — 팔 건 파는 결단력.", (s) => s.maxSell >= 10 * JO && s.maxSell < 50 * JO),
  S("n-maxsell-t3", "note", "win", "대형 방출", "최고가 방출 50만 이상 — 에이스급도 값이 맞으면 보냅니다.", (s) => s.maxSell >= 50 * JO),
  S("n-maxsell-half", "note", "win", "한 방 정리", "방출 수입 절반이 단 한 건 — 제값 받고 보냈습니다.", (s) => s.sellCount >= 3 && s.maxSell >= s.totalSell * 0.5),

  // ═══════════ 평균 단가 (note) ═══════════
  S("n-avgbuy-t1", "note", "info", "가성비 쇼핑", "영입 평균 단가 1만 미만 — 저평가 카드 발굴형.", (s) => s.buyCount >= 3 && s.avgBuy < 1 * JO),
  S("n-avgbuy-t2", "note", "info", "실속 쇼핑", "영입 평균 단가 1~10만 — 중저가 시장의 단골.", (s) => s.buyCount >= 3 && s.avgBuy >= 1 * JO && s.avgBuy < 10 * JO),
  S("n-avgbuy-t3", "note", "info", "프리미엄 쇼핑", "영입 평균 단가 10~50만 — 검증된 카드 위주로 삽니다.", (s) => s.buyCount >= 3 && s.avgBuy >= 10 * JO && s.avgBuy < 50 * JO),
  S("n-avgbuy-t4", "note", "gold", "명품관 단골", "영입 평균 단가 50만 이상 — 카탈로그 맨 윗줄만 봅니다.", (s) => s.buyCount >= 3 && s.avgBuy >= 50 * JO),
  S("n-avgsell-high", "note", "win", "고가 판매 장인", "방출 평균 단가 10만 이상 — 헐값에 넘기지 않습니다.", (s) => s.sellCount >= 3 && s.avgSell >= 10 * JO),
  ...
  S("n-combo-bigboth", "note", "gold", "큰물 트레이더", "지출과 수입 모두 100만 이상 — 시장의 유동성 공급자.", (s) => s.totalBuy >= 100 * JO && s.totalSell >= 100 * JO),
  ...
  // ═══════════ 최저가/헐값 (note) ═══════════
  S("n-minbuy-tiny", "note", "info", "천원샵 발견", "1,000 BP 미만 영입 기록 — 잡초 속 보석을 노리는 눈썰미.", (s) => s.minBuy > 0 && s.minBuy < 0.1 * JO),
  S("n-minbuy-none-cheap", "note", "gold", "저가 카드 무관심", "가장 싼 영입도 10만 이상 — 애초에 싼 매물은 안 봅니다.", (s) => s.buyCount >= 3 && s.minBuy >= 10 * JO),
  S("n-minsell-tiny", "note", "info", "떨이 처분", "1,000 BP 미만 방출 기록 — 자리 정리가 우선이었군요.", (s) => s.minSell > 0 && s.minSell < 0.1 * JO),
  ...
  S("n-combo-jackpot", "note", "gold", "잭팟 한 건", "단일 방출로 50만 이상 — 그날은 발 뻗고 주무셨겠어요.", (s) => s.maxSell >= 50 * JO && s.totalSell > 0 && s.maxSell >= s.totalSell * 0.7),
