...
- 방침·약관·푸터·내 정보 문의 메일을 **humsleep@naver.com** 으로 통일, 방침 11항에 보호책임자 성명(안혁) 명시.
  iOS `Config.contactEmail` 도 변경(심사 중인 Build 12 에는 미포함 — 다음 빌드부터).
- 공용 안내 사이트(humsleep.github.io/apps)에 FC Scope 를 외부 링크로 등록. 방침 본문은 계속 fcscope.xyz 에 둔다.
- ⚠️ 도메인 의존: 앱 API 베이스·유니버설 링크·방침 URL 이 전부 `www.fcscope.xyz`. 도메인 유지가 불확실하면
  다음 업데이트에서 API 베이스를 도메인 비의존 주소로 바꾸는 작업이 선행돼야 한다.

## 2026-09-22 — 상단 배너 배치 · 최종 검수 3종(코드 리뷰·웹 QA·iOS E2E) → GO

- 배너를 카카오톡식으로 모든 화면(홈·전적·매치·픽 랭킹·커뮤니티 목록/상세·내 정보) **상단 카드 1개**로 이동(맨 아래는 노출이 거의 없었다).
  AdMob 정책 리스크 보정: SDK 시작이 결정된 뒤에만 실제 적응형 높이로 자리 선점(레이아웃 점프·빈 카드 둘 다 방지),
  콘텐츠 없는 화면(빈 탭·차단 글·0경기 모드)엔 광고 없음, 배너 클리핑 제거(AdChoices), 재로드 시 광고 유지.
- 최종 검수: 코드 리뷰 차단 0 / 웹 QA 이전 9건 전부 수정 확인 / iOS E2E 10항목 PASS(온보딩 회귀 5회 재현 없음) → 빌드 11 제출.
- 부수: og:url/title(메타·커뮤니티·글 상세), 커뮤니티 행간, spid.json name 누락 방어, 몰수 결과 폴백 테스트, 운영자 시드 글 SQL(fcscope-ios/docs/seed-posts.sql).

## 2026-09-20 — 랭커 데이터 복구 · 공유 카드 v2 · 출시 전 4중 최종 테스트

- 🔴 **랭커 데이터 전량 유실**: 넥슨 ranker-stats 는 선수 ID 를 `spid`(소문자)로 준다 → `spId` 로 읽어 키가 "undefined:25".
  사용자가 직접 넥슨 응답을 떠 와서 확정. 정규화 + 옛 tombstone 무효화(v2).
- **픽 랭킹 기준**: ranker-stats `matchCount` 는 조합마다 20 고정 → 인기 순위로 무의미. 크론이 match_cache 선발 횟수(`usage`)를
  스냅샷에 붙이고 그 값으로 정렬. 자정 직후 usage 없는 날을 고르던 버그, 400행 컷에 인기 카드가 잘리던 버그도 수정.
  "랭커가 많이 쓴다" 문구는 사실이 아니라 전부 "선발 횟수 + 랭커 성적"으로 교체.
- 최종 테스트(코드 리뷰·데이터 감사·웹 QA·iOS E2E)에서 잡은 것:
  - 골 코드 판별이 자책골 경기에서 유효슛(1)을 골로 오판 → 3 고정 + 회귀 테스트
  - 헤딩·FK·PK 는 박스 안/밖의 부분집합 → 합산하면 슛이 이중 계산
  - "최근 7일"이 30경기 상한에 걸린 경우 `week.truncated`, 교체 투입 선수 누락, 몰수 경기 결과 "?"
  - 온보딩 확인(stage=profile)이 search_log 에 남아 타이핑 중 문자열이 홈 칩에 공개
  - 공지 배너 가로 스크롤(inline span truncate), 전 페이지 canonical 이 홈
- 교훈: 외부 API 필드는 **실제 응답을 한 번 떠서** 타입과 대조할 것. 이번 두 건(spid, matchCount=20) 모두 문서만 믿어 생겼다.

## 2026-09-19 — 팀 프리셋 20팀 · 출시 전 3중 점검(심사·유저·보안)

- **팀 프리셋 6 → 20팀**(EPL 8·라리가 3·세리에 4·분데스 2·리그1·국가대표 2, 2025-26 대표 선발).
  이름 부분일치(`resolvePlayer`)는 "디아스"처럼 동명이인을 잘못 집었다 → `PresetPlayer.pid` 로 고정(spid.json 으로 220명 대조).
- **랭커 데이터 빈 상태 진단**: 크론 응답·로그에 `probes`(넥슨 ranker-stats 1콜 결과/에러 코드) 추가.
  프로덕션 `/api/v1/player/*` 의 ranker 가 손흥민 카드까지 비어 있어 넥슨 응답 자체를 봐야 한다.
- 점검 결과(앱 반영): ATT 를 첫 전적 화면에서 표시(두 번째 세션 조건 제거 — 심사자가 못 봄),
  홈 "EA SPORTS FC ONLINE DATA LAB" → "FC온라인 비공식 데이터 랩", 딥링크 ID 검증·유니버설 링크 호스트 정확 일치.
- 보안 후속(출시 후): Supabase Email 가입 끄기, `profiles` 컬럼 SELECT 축소, 커뮤니티 금칙어를 DB 트리거로, security definer search_path 고정.

## 2026-09-18 — [진단 정확성] 평점 척도·후반 시간·몰수 경기 · 시즌 아이콘 · 선수 사진 선택

출시 전 비판 검토(Reality Checker)에서 "진단이 틀리고 있다"는 지적이 나왔고, 운영 데이터로 전부 사실임을 확인했다.
테스트 379 → **436 PASS**.

- 🔴 **평점 척도**: `matchDetail.averageRating` 은 출전 선수 평균이 아니라 **엔트리 18명(벤치 포함) 평점 합 ÷ 18** 이었다
  (159경기 전부 합/avg = 18.00). 값이 3~5에 눌려 있는데 기준을 6.5로 잡아 **샘플 전원이 "분발 필요"**, 2:0 승리가 "고전한 경기".
  → 실제로 뛴 선수 평균(`lib/nexon/rating.ts`)으로 교체(중앙값 6.71), 스코어 재보정. 승률 50%대 = 평범, 60%+ = 수준급.
- 🔴 **goalTime**: 후반 = 2^24 + 초, 연장 = 2^25(+2^24) + 초. 그대로 60으로 나눠 슛 시각이 "279622분",
  후반 골이 전부 76-90+ 구간으로 몰려 리포트 문장("후반 막판 실점 46%")이 거짓이었다. `lib/nexon/goal-time.ts`.
- **몰수 경기**(표본 17%)가 0:3/3:0 으로 대패·클린시트·"롤러코스터" 판정에 섞였다 → 득실·유형 진단에서 제외(승패·승률엔 유지).
- **랭커 크론**: 09-08 match_cache 패킹 이후 크론이 payload 를 풀지 못해 예열 조합이 0이었다 → 수정.
  단, 넥슨 ranker-stats 직접 조회도 비어 있어 넥슨 측 원인일 가능성이 남음(배포 후 크론 응답 확인 필요).
- 교훈: "그럴듯한 숫자"는 실측 분포로 검증하기 전까지 믿지 말 것. 기준값(6.5)은 웹 시절 가정이었다.

앱: 시즌 약어 대신 **넥슨 공식 시즌 아이콘**(seasonid.json, 키 불필요)을 표시하고, 스쿼드에서 **같은 선수(pid)의
다른 시즌 사진**을 고를 수 있게 했다(`SquadSlot.imageSpid`, 서버도 같은 pid 만 허용 — 계약에 옵셔널 추가).

## 2026-09-15 — [출시 감사] 백엔드 차단 요소 수정 · Apple 토큰 폐기 · 방침 v3

출시 전 최종 감사(심사 규정·크래시·보안·UX 5개 영역)에서 나온 백엔드 항목을 처리했다. 테스트 266 → **379 PASS**.

- **없는 구단주명이 502 → 404 `user_not_found`.** 넥슨은 없는 닉네임에 `400 + OPENAPI00004`를 준다(00003 만 보고 있었다).
  `/id` 단계에서만 파라미터 오류를 not-found 로 번역한다(`lib/nexon/errors.ts`) — 다른 단계에 쓰면 우리 버그를 숨긴다.
  `/api/v1/match/[id]` 는 24자리 hex 가 아니면 넥슨을 부르지 않고 400. 운영 확인: 404 / 400.
- **UGC 금칙어 필터**(App Store 1.2): 글·댓글·닉네임. 한 글자씩 띄어쓰기·기호 삽입·영타 우회 대응, 시발점 등 허용 목록.
- **푸시 안전장치**: `APNS_SANDBOX` 는 `1`/`true` 만 샌드박스(`0` 도 샌드박스로 가서 첫 크론에 토큰 전량 삭제가 가능했다),
  http2 세션 error/goaway 처리(리스너 없으면 프로세스가 죽는 것을 로컬 재현), 무효 토큰 50% 초과 시 삭제 중단.
- **`/api/v1/devices`** IP 20회/분, 즐겨찾기 항목 길이 제한, `last_seen` 순 발송 대상.
- **0021 컬럼 단위 권한**: RLS 는 "어느 행"만 막는다. 작성자가 `created_at` 을 과거로 PATCH 해 도배 제한을 우회하거나
  `verified_*` 를 직접 써서 구단주 연동을 사칭할 수 있었다. consented_at·verified_* 쓰기는 service_role 로 이동.
- **0022 squads 공개 SELECT 제거**: anon REST 로 `ip_hash`·`user_id` 가 읽혔다. 마이페이지 내 스쿼드는 service_role 로 읽는다.
- **Sign in with Apple 토큰 폐기**(5.1.1(v)): 삭제 요청 본문의 `appleAuthorizationCode` → `/auth/token` → `/auth/revoke`.
  키(`APPLE_SIWA_KEY_ID`·`APPLE_SIWA_PRIVATE_KEY`)가 없거나 실패해도 삭제는 막지 않는다.
- **개인정보처리방침 v3(2026-09-23 시행 — 7일 전 공지 약속 때문에 9/16 공지 기준)**: 앱이 실제로 요청하는데 "요청하지 않음"으로 적혀 있던 알림·사진 추가 권한,
  알림용 구단주명·즐겨찾기, 투표 해시를 고지. 감사에서 코드와 방침을 대조하다 발견했다.
- AASA 에서 `/terms*` `/privacy*` 제외(앱 안 방침 링크가 앱으로 되돌아오지 않게). events 요청당 상한 20.
- 배포 순서가 중요한 마이그레이션: **코드 배포 → 0021 → 0022**. 반대로 하면 구 코드의 연동·동의 기록이 42501 로 실패한다.
- App Store 스크린샷 촬영 중 발견: `/api/v1/player/[spid]` 의 `season` 이 요청 카드가 아닌 대표 카드 시즌이었다(PTG 카드에 "26 TOTS").
  요청 spid 의 시즌으로 수정. 앱 팀 프리셋 18팀 중 12팀이 서버 `PRESETS`(6팀)에 없어 404 "not found" 영문이 떴다 →
  앱 목록을 6팀으로 줄이고 프리셋 라우트 오류 문구를 한국어로.
- ⚠️ 남은 한계: 팀 프리셋은 선수 **이름**으로 spid 를 찾아서 동명이인을 잘못 고른다(레알 "로드리고" → 다른 팀 로드리고 22 PL 카드).
  제대로 하려면 `presets.ts` 에 이름 대신 pid 를 박아야 한다.

## 2026-09-15 — [앱 측정] 익명 사용 이벤트 수집 (`POST /api/v1/events`)

출시 후 "자랑하고 싶게, 계속 쓰고 싶게"가 실제로 되는지 판단할 근거가 없어서 넣었다.
재방문(D1/D7), 공유 카드 만들기→공유 전환, 검색 전면광고가 검색을 줄이는지, 전적 화면에서 어떤 보기를 쓰는지.

- **테이블 `app_events`**(마이그레이션 `0020`) — RLS 켜고 정책 없음(서비스 롤만 쓰기). 180일 뒤 `ranker-snapshot` 크론이 지운다.
- **`lib/analytics/events.ts`** — 이벤트 이름 허용 목록(10종), 요청당 최대 50건, installId 는 UUID 만,
  props 는 원시값 8개·키 `^[a-z_]{1,24}$`·문자열 60자, 시각은 미래 5분·과거 7일로 클램프.
- **라우트는 DB 실패에도 200** — 측정 때문에 앱이 재시도 폭주하거나 오류를 보이면 안 된다.
  대신 응답만으로는 저장 성공을 알 수 없으니 확인은 DB 에서 한다.
- 계정·이메일·구단주명·IDFA·IDFV 는 보내지 않는다. 설치 시 만든 무작위 UUID 하나만(앱 삭제 시 소멸).
  `env`(debug/testflight/appstore)로 개발·TestFlight 기록을 통계에서 뺀다.
- 통계 쿼리는 `scripts/analytics.sql`. 개인정보처리방침에 "앱 이용 기록 · 180일" 추가.
- 단위 테스트 16종 추가(250 → **266 PASS**). 계약에 `POST /api/v1/events` 추가.
- 프로덕션 호출 확인: `{"ok":true,"accepted":1}` (env=debug 프로브 1건).
- 앱 쪽: 이 작업 중 **`PrivacyInfo.xcprivacy` 누락**을 발견해 본체·위젯에 추가했다(없으면 App Store 업로드 거부).
  검색 전면광고의 배포판 광고 단위도 설정 완료.

## 2026-09-09 — [계약 검증] 앱이 디코딩하는 응답을 깨뜨리면 잡히게 만들었다

저장소를 나누면서(웹 GitHub `fconline` / 앱 GitHub `fcscope`) 생긴 구멍을 막았다.
앱은 12개 v1 라우트를 `Models.swift`(420줄)로 그대로 디코딩하는데 **버전 협상이 없다**.
Swift `Decodable` 은 필수 필드가 하나만 없어도 **응답 전체 디코딩이 실패**하고,
앱스토어에 나간 구버전은 몇 달씩 살아 있어 심사 때문에 즉시 고칠 수도 없다.
즉 웹에서 필드 하나를 "안 쓰는 것 같아서" 지우면 배포된 앱이 그 순간 죽는다.
(실제로 이번 효율화에서 `match_cache.ouids` 를 같은 논리로 지웠다 — v1 에서 그러면 사고가 된다.)

- **`lib/api/contract.ts`** — 앱이 반드시 받아야 하는 필드 트리를 선언. 앱의 `Models.swift` 에서 그대로 옮겼고,
  옵셔널 여부까지 반영했다. 필드 **추가는 위반이 아니다**(앱은 모르는 키를 무시한다).
- **`npm run verify:api [baseUrl]`** — 실제 서버를 호출해 대조. 프로덕션 실행 결과 **11/11 라우트 위반 0건**.
  경로 파라미터(matchId·spid·postId)는 앞선 응답에서 실제 값을 뽑아 쓴다.
- **단위 테스트 21종 추가**(229 → **250 PASS**) — 검증기 자체 + 계약 정의 무결성
  (참조하는 스펙 이름이 전부 존재하는지. 오타 하나로 검증이 조용히 무력화되는 걸 막는다).
- 실제 프로덕션 응답에 파괴적 변경 5종을 가해 전부 잡히는 것을 확인:
  필드 삭제 · 필드 개명 · int→소수 · 필수 필드 null · 중첩 배열 원소의 필드 삭제.
- AGENTS.md 에 "🔴 `/api/v1` 은 깨뜨릴 수 없는 공개 계약 — 필드 추가만 허용" 원칙을 추가.
  없앨 필드는 지우지 말고 `null` 로 계속 내려보내고, 정말 깨야 하면 `/api/v2` 를 새로 만든다.
- 앱은 v1 외에 `/api/players/search`·`/api/squad/*`·`/api/profile`·`/api/me/notifications`·
  `/api/community/battle` 등 **구 라우트도 그대로 쓴다**. 이쪽이 오히려 웹 리팩터링에 휩쓸리기 쉬워
  전부 계약에 넣었다 — **계약 21개, 자동 검증 16개**(로그인 필요 1 · 쓰기 4는 명시적으로 건너뛴다).
  프로덕션 실행 결과 **16/16 위반 0건**.
- 🔴 **계약을 넣자마자 실제 버그가 하나 잡혔다**: 앱 `BattleVotes` 가 CodingKeys 로 `a`→`"A"`, `b`→`"B"`
  대문자에 매핑돼 있었다. 서버는 소문자 `{"a":0,"b":0}` 를 준다(웹 `BattleVote.tsx` 도 소문자를 읽는다).
  옵셔널이라 **디코딩은 성공하고 값만 항상 nil** → 스쿼드 배틀 투표수가 늘 "A 0표 / B 0표" 로 보였다.
  또 서버가 주지 않는 `mine` 으로 버튼 비활성화를 판단해 투표 후에도 버튼이 계속 열려 있었다.
  앱 저장소에서 수정(소문자 디코딩 + `myPick` 로컬 상태). iOS 빌드 ✓.
  타입이 옵셔널이면 계약 위반이 조용히 지나간다는 사례라, 앱 모델의 옵셔널 남용도 비용이 있다.
...
- **P0 발견성(Lens1)**: 스쿼드 빌더가 닉네임 임포트(`/api/squad/from-user`)를 지원하는데 선수 성적표 탭에 진입 경로가 없던 숨은 기능 → 성적표에 "🛡️ 이 스쿼드 빌더로 열기" CTA(`/squad?owner=`) + SquadBuilder `?owner=` 딥링크(`?load=` 미러, `loadMySquad(nickArg?)`로 setState 레이스 회피). 실측 프리필+로드.
- **P0 첫방문(Lens2)**: 히어로 칩 선수 검색·픽 랭킹이 둘 다 /meta라 중복(지난주 회귀) → 중복 제거 + 픽 랭킹→"픽 랭킹·선수" 라벨 + 빈 슬롯에 "⚔️ 스쿼드 배틀"(/community?type=squad_battle). 5칩 5목적지.
- **P1 재방문(Lens3)**: 주간 성적표 리캡 — 순수 `lib/nexon/weekly.ts`가 이미 로드된 summaries를 최근 7일 윈도잉(승패·승률·최다연승·평균 스코어·최고 경기) → `/user` 배너(주 3경기+) + `/api/card/weekly/[nickname]` 공유 카드(rival/streak 파이프라인 복제). Wrapped식 정기 재방문 훅. 마이그레이션·추가 팬아웃(페이지) 0.
- 검증: `tsc` 0 · **243 PASS**(+12 weeklyRecap) · build ✓(weekly 라우트) · Playwright 390px 5칩·오버플로 0·`/squad?owner=` 프리필+로드. PR #94
- **다음 후보**: 홈 대시보드 폼 델타 배지(로그인) · 주간 메타 공유 카드(loadPicks+renderCard) · 선수 상세 "배치" CTA `?spid=` 프리필 · 극소액 거래 "0" 표시(LOW) · 시즌 테마 카드.
