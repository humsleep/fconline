...
export interface MatchPerfStats {
  /** 결과가 확정된 전체 경기(몰수 포함) — 승/무/패·승률·연승/연패·모멘텀의 분모 */
  played: number;
  /**
   * 정상 종료 경기 수(몰수 제외). 득실·클린시트·대승/대패·접전·점유율·평점 등
   * 스코어 기반 지표는 전부 이 표본에서만 센다 — 몰수는 3:0/0:3 으로 기록돼 대승·클린시트를 오염시킨다.
   */
  normalPlayed: number;
  /** 정상 종료 경기의 승리 수(몰수승 제외) — 접전승 비율 등의 분모 */
  normalWin: number;
  win: number;
  draw: number;
  lose: number;
...
export function computeMatchPerfStats(summaries: MatchSummary[]): MatchPerfStats {
  ...
  let normalPlayed = 0, normalWin = 0;

  for (const m of ms) {
    if (m.result === "승") win++;
    else if (m.result === "무") draw++;
    else lose++;
    // 몰수: 승패·연속 기록에는 남기고, 스코어 기반 지표에서는 뺀다(기록상 3:0/0:3 · 평점도 중단 시점 값).
    if (m.forfeit) {
      forfeits++;
      continue;
    }
    normalPlayed++;
    if (m.result === "승") normalWin++;
    const gf = m.me.goals;
  ...
  return {
    played, normalPlayed, normalWin, win, draw, lose, winRate,
    goalsFor, goalsAgainst,
    avgFor: normalPlayed ? goalsFor / normalPlayed : 0,
    avgAgainst: normalPlayed ? goalsAgainst / normalPlayed : 0,
...
export const MATCH_RULES: MatchRule[] = [
  ...
  R("t-wall", "type", "win", "철벽 수비", "경기당 실점 1 미만 + 무실점 경기 다수 — 수비가 팀의 정체성입니다.", (s) => s.normalPlayed >= 10 && s.avgAgainst < 1 && s.cleanSheets >= s.normalPlayed * 0.4),
  R("t-cannon", "type", "gold", "화력 덕후", "경기당 3골 이상 — 일단 상대보다 한 골 더 넣으면 됩니다.", (s) => s.normalPlayed >= 10 && s.avgFor >= 3),
  R("t-counter", "type", "win", "역습의 명수", "점유율을 내주고도 이깁니다 — 한 방 카운터가 주무기.", (s) => s.played >= 10 && s.counterWins >= 3 && s.winRate >= 50),
  R("t-clutch", "type", "win", "클러치 승부사", "1골차 승리가 유독 많습니다 — 접전에서 강한 심장.", (s) => s.normalPlayed >= 10 && s.closeWins >= s.normalWin * 0.5 && s.normalWin >= 5),
  R("t-dominator", "type", "info", "점유율 신봉자", "평균 점유율 55% 이상 — 공을 소유해야 마음이 편한 타입.", (s) => s.normalPlayed >= 10 && s.avgPossession >= 55),
  ...
  // ═══════════ 득점력 (note) ═══════════
  R("n-gf-4", "note", "gold", "경기당 4골+", "폭격기 수준의 화력 — 상대 골키퍼에게 묵념.", (s) => s.normalPlayed >= 5 && s.avgFor >= 4),
  R("n-gf-3", "note", "win", "경기당 3골대", "꾸준한 다득점 — 공격 루트가 다양하다는 뜻.", (s) => s.normalPlayed >= 5 && s.avgFor >= 3 && s.avgFor < 4),
  R("n-gf-2", "note", "info", "경기당 2골대", "평균 이상의 득점력 — 해결사가 있군요.", (s) => s.normalPlayed >= 5 && s.avgFor >= 2 && s.avgFor < 3),
  R("n-gf-1", "note", "info", "경기당 1골대", "득점이 아쉬운 날이 있습니다 — 마무리 정확도가 관건.", (s) => s.normalPlayed >= 5 && s.avgFor >= 1 && s.avgFor < 2),
  R("n-gf-low", "note", "lose", "경기당 1골 미만", "창끝이 무딥니다 — 공격 전개 패턴 점검이 필요해요.", (s) => s.normalPlayed >= 5 && s.avgFor < 1),
  R("n-max-6", "note", "gold", "한 경기 6골+", "최다 득점 경기가 6골 이상 — 그날 상대는 악몽을 꿨을 겁니다.", (s) => s.maxGoals >= 6),

  // ═══════════ 수비 (note) ═══════════
  R("n-ga-low", "note", "win", "경기당 실점 1 미만", "잠그는 축구의 정석 — 수비 조직력이 훌륭합니다.", (s) => s.normalPlayed >= 5 && s.avgAgainst < 1),
  R("n-ga-1", "note", "info", "경기당 실점 1점대", "평균적인 수비 — 무실점 경기를 늘리면 승률이 뜁니다.", (s) => s.normalPlayed >= 5 && s.avgAgainst >= 1 && s.avgAgainst < 2),
  R("n-ga-2", "note", "lose", "경기당 실점 2점대", "뒷문이 자주 열립니다 — 수비 라인 간격을 점검하세요.", (s) => s.normalPlayed >= 5 && s.avgAgainst >= 2 && s.avgAgainst < 3),
  R("n-ga-high", "note", "lose", "경기당 실점 3+", "수비 리빌딩이 시급합니다 — 실점 장면 리플레이 추천.", (s) => s.normalPlayed >= 5 && s.avgAgainst >= 3),
  R("n-cs-many", "note", "win", "무실점 경기 40%+", "클린시트 비율이 최상위권 — 골키퍼와 수비진에 보너스를.", (s) => s.normalPlayed >= 5 && s.cleanSheets >= s.normalPlayed * 0.4),
  R("n-cs-some", "note", "info", "클린시트 보유", "무실점 경기가 있습니다 — 잠글 줄 아는 팀.", (s) => s.normalPlayed >= 5 && s.cleanSheets >= 2 && s.cleanSheets < s.normalPlayed * 0.4),
  ...
  R("n-scoreless-many", "note", "lose", "무득점 경기 다수", "0골 경기가 잦습니다 — 공격 옵션을 늘려보세요.", (s) => s.normalPlayed >= 5 && s.scoreless >= s.normalPlayed * 0.3),
  R("n-bothscored", "note", "info", "화끈한 오픈 게임", "양팀 모두 득점한 경기가 대부분 — 닫아두는 법이 없군요.", (s) => s.normalPlayed >= 5 && s.bothScored >= s.normalPlayed * 0.7),
  ...
  // ═══════════ 조합 (note) ═══════════
  R("n-combo-fortress", "note", "win", "이기면 무실점", "승리 경기 대부분이 클린시트 — 앞서면 잠급니다.", (s) => s.normalWin >= 5 && s.cleanSheets >= s.normalWin * 0.6),
  R("n-combo-shootout", "note", "info", "난타전 체질", "경기당 합계 4골 이상 — 보는 사람은 즐겁습니다.", (s) => s.normalPlayed >= 5 && (s.goalsFor + s.goalsAgainst) / s.normalPlayed >= 4),
  R("n-combo-grind", "note", "info", "저득점 승부사", "경기당 합계 2골 미만의 실리 축구 — 한 골 싸움에 강해야 합니다.", (s) => s.normalPlayed >= 5 && (s.goalsFor + s.goalsAgainst) / s.normalPlayed < 2),
  ...
  R("n-combo-winhigh-gflow", "note", "info", "효율의 마법사", "득점은 적은데 승률은 높습니다 — 필요한 골만 넣는 효율파.", (s) => s.normalPlayed >= 10 && s.winRate >= 55 && s.avgFor < 2),
  R("n-combo-gfhigh-wrlow", "note", "info", "골은 많은데…", "다득점에도 승률이 아쉽습니다 — 실점 관리가 발목을 잡네요.", (s) => s.normalPlayed >= 10 && s.avgFor >= 2.5 && s.winRate < 50),
  R("n-combo-nodraw", "note", "info", "무승부 없음", "비긴 경기가 하나도 없습니다 — 끝장을 봐야 직성이 풀리는 타입.", (s) => s.played >= 10 && s.draw === 0),
  R("n-combo-balanced-gd", "note", "info", "종이 한 장 차이", "득실차가 ±3 이내 — 모든 경기가 접전이었습니다.", (s) => s.normalPlayed >= 10 && Math.abs(s.goalsFor - s.goalsAgainst) <= 3),
  R("n-combo-decisive", "note", "win", "확실한 마무리", "승리의 절반 이상이 2골차 이상 — 이길 땐 여유 있게 이깁니다.", (s) => s.normalWin >= 5 && s.normalWin - s.closeWins >= s.normalWin * 0.5 && s.winRate >= 50),
  ...
  // ═══════════ 골 패턴 추가 (note) ═══════════
  R("n-goal-everygame", "note", "win", "매 경기 득점", "무득점 경기가 없습니다 — 어떤 수비든 한 번은 뚫습니다.", (s) => s.normalPlayed >= 5 && s.scoreless === 0),
  R("n-goal-nocs", "note", "lose", "클린시트 제로", "무실점 경기가 없습니다 — 매 경기 한 골은 내준다는 뜻.", (s) => s.normalPlayed >= 5 && s.cleanSheets === 0),
  R("n-goal-max45", "note", "info", "한 경기 4~5골", "최다 득점 경기 4~5골 — 터질 땐 확실히 터집니다.", (s) => s.maxGoals >= 4 && s.maxGoals <= 5),
  R("n-goal-avg5", "note", "gold", "경기당 5골+", "이건 축구가 아니라 농구입니다 — 압도적 공격력.", (s) => s.normalPlayed >= 5 && s.avgFor >= 5),
  R("n-goal-oneside", "note", "info", "원사이드 경기 다수", "한쪽만 득점한 경기가 대부분 — 흐름을 잡은 쪽이 다 가져갑니다.", (s) => s.normalPlayed >= 5 && s.bothScored <= s.normalPlayed * 0.3),
  R("n-goal-maxcon34", "note", "info", "한 경기 3~4실점", "실점이 몰린 경기가 있었습니다 — 그날의 상성을 복기해보세요.", (s) => s.maxConceded >= 3 && s.maxConceded <= 4),

  // ═══════════ 스타일 조합 추가 (note) ═══════════
  R("n-style-complete", "note", "gold", "완전체 밸런스", "경기당 2.5골 이상 넣고 1.2골 미만 실점 — 공수 모두 최상위권.", (s) => s.normalPlayed >= 10 && s.avgFor >= 2.5 && s.avgAgainst < 1.2),
  R("n-style-shieldnosword", "note", "info", "창보다 방패", "실점은 적은데 득점도 적습니다 — 공격에 한 명만 더 투자하면?", (s) => s.normalPlayed >= 10 && s.avgAgainst < 1.2 && s.avgFor < 1.5),
  R("n-style-grindwin", "note", "info", "꾸역승 전문", "대승 없이 꾸준히 이깁니다 — 어떻게든 이기는 게 실력입니다.", (s) => s.normalWin >= 5 && s.bigWins === 0 && s.winRate >= 50),
  R("n-style-nocollapse", "note", "win", "대패 없음", "3골차 이상으로 진 적이 없습니다 — 무너지지 않는 팀.", (s) => s.normalPlayed >= 10 && s.bigLosses === 0),
